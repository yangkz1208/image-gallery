...
function bind() {
  ...
  document.querySelectorAll(".folder-card").forEach(c => {
    ...
    c.ondrop = async e => {
      e.preventDefault(); c.style.border="2px solid #eee";
      if(!STATE.isAdmin) return alert("仅管理员可上传");
      for(let f of e.dataTransfer.files) f.type.startsWith("image/") && await upload(c.dataset.folder,f);
  ...
  try {
    ...
    if(i === real) {
      ...
      if(STATE.loginAttempts >= CONFIG.maxLoginAttempts) {
        ...
        alert("尝试过多，已锁定5分钟");
      } else alert(`密码错误，剩余${CONFIG.maxLoginAttempts-STATE.loginAttempts}次`);
    }
  } catch(e) { alert("验证失败"); }
}
...
async function loadImgs(f) {
  ...
  fs.filter(x=>x.type=="file"&&/\.(jpg|jpeg|png|gif|webp)$/i.test(x.name)).forEach(file=>{
    ...
    it.className="image-item"; it.dataset.path=p;
...
async function delSel() {
  ...
  for(let p of STATE.selectedImages) {
    if(STATE.lockedImages.includes(p)) { alert(p.split("/")[1]+"已锁定"); continue; }
...
async function upload(f,file) {
  ...
  rd.onload = async () => {
    ...
    alert("上传成功");
    await loadImgs(f); await calcTotal();
  };
}
